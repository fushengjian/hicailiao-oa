...
<script type="text/javascript">
	$("#${id}Button").click(function(){
        ...
        var winOpen = top; if(winOpen == null)winOpen = this;
		// 正常打开	
		winOpen.$.jBox.open("iframe:${ctx}/tag/treeselect.do?url="+encodeURIComponent("${url}")+"&module=${module}&checked=${checked}&extId=${extId}&nodesLevel=${nodesLevel}&selectIds="+$("#${id}Id").val(), "选择${title}", 300, 420, {
			buttons:{"确定":"ok", ${allowClear?"\"清除\":\"clear\", ":""}"关闭":true}, submit:function(v, h, f){
				if (v=="ok"){
					...
					for(var i=0; i<nodes.length; i++) {//<c:if test="${checked}">
						...
						if (nodes[i].level == 0){
							winOpen.$.jBox.tip("不能选择根节点（"+nodes[i].name+"）请重新选择。");
							return false;
						}//</c:if><c:if test="${notAllowSelectParent}">
						if (nodes[i].isParent){
							winOpen.$.jBox.tip("不能选择父节点（"+nodes[i].name+"）请重新选择。");
							return false;
						}//</c:if><c:if test="${not empty module && selectScopeModule}">
						if (nodes[i].module == ""){
							winOpen.$.jBox.tip("不能选择公共模型（"+nodes[i].name+"）请重新选择。");
							return false;
						}else if (nodes[i].module != "${module}"){
							winOpen.$.jBox.tip("不能选择当前栏目以外的栏目模型，请重新选择。");
							return false;
						}//</c:if>
						ids.push(nodes[i].id);
                        var t_node = nodes[i];
				...
				$(".jbox-content", winOpen.document).css("overflow-y","hidden");
			}
		});
	});
</script>
