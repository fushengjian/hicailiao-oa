...
<body>
	<ul class="nav nav-tabs">
		...
		<li class="active"><a href="${ctx}/sys/role/assign.do?id=${role.id}"><shiro:hasPermission name="sys:role:edit">角色分配</shiro:hasPermission><shiro:lacksPermission name="sys:role:edit">人员列表</shiro:lacksPermission></a></li>
	</ul>
	...
	<div class="breadcrumb">
		...
		<script type="text/javascript">
			$("#assignButton").click(function(){
				top.$.jBox.open("iframe:${ctx}/sys/role/usertorole.do?id=${role.id}", "分配角色",810,$(top.document).height()-240,{
					buttons:{"确定分配":"ok", "清除已选":"clear", "关闭":true}, bottomText:"通过选择部门，然后为列出的人员分配角色。",submit:function(v, h, f){
						...
						if (v=="ok"){
					    	...
					    	$('#assignRoleForm').attr('action','${ctx}/sys/role/assignrole.do?id=${role.id}&idsArr='+idsArr).submit();
					    	return true;
						} else if (v=="clear"){
		...
		<c:forEach items="${users}" var="user">
			<tr>
				...
				<shiro:hasPermission name="sys:role:edit"><td>
					<a href="${ctx}/sys/role/outrole.do?userId=${user.id}&roleId=${role.id}" 
