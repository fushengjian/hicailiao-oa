...
public class DatabaseTools implements IBackup {
	private static Logger log = LoggerFactory.getLogger(DatabaseTools.class);
	...
	public boolean createSqlScript(HttpServletRequest request, String path,
		...
		if (size != null) {
			try {
				psize = Float.parseFloat(size);
			} catch (NumberFormatException ignore) {
			}
		}
		List<String> tablelists = this.publicMethod
				.getAllTableName("show tables");
		List<String> backup_list;
		...
		try {
			...
			for (String table : backup_list) {
				...
				for (String line : list) {
					// CommUtil.div(Long.valueOf(file.length()), Integer.valueOf(1024));
		...
		try {
			...
			for (String table : backup_list) {
				// CommUtil.null2String(table).equals("")
	...
	public boolean executSqlScript(String filePath) throws Exception {
		...
		try {
			...
			ret = false;
			conn.rollback();
			log.error("error executSqlScript", ex);
		}
		return ret;
	}
	...
	public List<String> getAllDatas(String tableName) throws Exception {
		...
		try {
			...
			StringBuilder columnsStr = new StringBuilder().append("describe ")
					.append(tableName);
			List<TableColumn> columnList = this.publicMethod
					.getDescribe(columnsStr.toString());
			...
			while (rs.next()) {
				...
				for (TableColumn dbColumn : columnList) {
					...
					if (rs.getString(columnsFile) == null) {
						...
						vector.add(Integer.valueOf(Integer.valueOf(
