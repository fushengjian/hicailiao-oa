	...
	var bigAutocomplete = new function(){
		...
		this.init = function(){
			...
			$("#bigAutocompleteContent").delegate("tr", "click", function() {
				bigAutocomplete.currentInputText.val( $(this).find("div:first").html());
		...
		this.autocomplete = function(param){
			...
			$this.keyup(function(event) {
				...
				if(!isFunctionalKey && (!ctrl || (ctrl && k == 67) || (ctrl && k == 88)) ){
					...
					var leftPad=-3
					if(config.topPad){
						topPad=config.topPad;
					}
					if(config.leftPad){
						leftPad=config.leftPad
					}
					var h = $this.outerHeight()+topPad;
					$bigAutocompleteContent.css({"top":offset.top + h,"left":offset.left+leftPad});
			...
			function makeContAndShow(data_){
				...
				for(var i=0;i<data_.length;i++){
					cont += "<tr><td><div style=\"float:left;\">" + data_[i].title + "</div><div  style=\"float:right;\">"+data_[i].count+"</div></td></tr>"
				}
				cont += "</tbody></table>";
