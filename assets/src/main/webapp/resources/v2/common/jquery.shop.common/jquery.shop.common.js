...
function populateForm() {
	var queryForm = jQuery("#queryForm");
	var listForm = jQuery("#ListForm");
	var queryCondition = listForm.find("#queryCondition");

	queryForm.find(":input").each(function() {
		if (this.type != "button" && this.type != "submit") {
			var hidden;
			var name = 'q_' + this.name;

			var hiddens = listForm.find(":input").filter("[name='" + name + "']");
			if (hiddens.length > 0) {
				hidden = hiddens[0];
...
jQuery(document).ready(function(){
  ...
  tipStyle();
  //
jQuery("a[dialog_uri],input[dialog_uri]").click(function(e){
	...
	if(dialog_uri!=undefined&&dialog_uri!=""){
		jQuery.ajax({type:'POST',url:dialog_uri,async:false,data:'',success:function(html){
			...
			jQuery(".white_content").css("position","absolute").css("top",parseInt(dialog_top)+"px").css("left",parseInt(dialog_left)+"px").css("border","10px rgb(85, 85, 85) solid").css("border-radius","13px");
